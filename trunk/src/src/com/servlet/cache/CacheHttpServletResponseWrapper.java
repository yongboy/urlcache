...
/**
 * 缓存的HttpServletResponseWrapper，它会把{@link HttpServletResponse}的部分数据记录到
 * {@link ResponseContent}里。
 */
public class CacheHttpServletResponseWrapper extends HttpServletResponseWrapper {
	...
	public CacheHttpServletResponseWrapper(HttpServletResponse response) {
		this(response, Long.MAX_VALUE, CacheFilter.EXPIRES_ON,
				CacheFilter.LAST_MODIFIED_INITIAL, -60);
	}

	public CacheHttpServletResponseWrapper(HttpServletResponse response,
			long time, long lastModified, long expires, long cacheControl) {
		super(response);
		...
		if (lastModified == CacheFilter.LAST_MODIFIED_INITIAL) {
			...
			super.setDateHeader(CacheFilter.HEADER_LAST_MODIFIED,
		...
		if (this.cacheControl == CacheFilter.MAX_AGE_TIME) {
			...
			super.addHeader(CacheFilter.HEADER_CACHE_CONTROL, "max-age=" + time
					/ 1000);
		} else if (this.cacheControl != CacheFilter.MAX_AGE_NO_INIT) {
			result.setMaxAge(this.cacheControl);
			super.addHeader(CacheFilter.HEADER_CACHE_CONTROL, "max-age="
					+ (-this.cacheControl));
		} else if (this.cacheControl == CacheFilter.MAX_AGE_NO_INIT) {
			result.setMaxAge(this.cacheControl);
		}

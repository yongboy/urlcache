...
public class ResponseContent implements Serializable {
	private static final long serialVersionUID = 23534647734657869L;
	
	private transient ByteArrayOutputStream bout = new ByteArrayOutputStream(
			1000);
	 ...
	 * @throws IOException
	 */
	public void writeTo(ServletResponse response, boolean fragment,
			...
			if (response instanceof HttpServletResponse) {
				...
				if (lastModified != -1) {
					httpResponse.setDateHeader(
							CacheFilter.HEADER_LAST_MODIFIED, lastModified);
				}

				// add the expires header
				if (expires != Long.MAX_VALUE) {
					httpResponse.setDateHeader(CacheFilter.HEADER_EXPIRES,
							expires);
				}

				// add the cache-control header for max-age
				if (maxAge == CacheFilter.MAX_AGE_NO_INIT
						|| maxAge == CacheFilter.MAX_AGE_TIME) {
					// do nothing
				} else if (maxAge > 0) { // set max-age based on life time
					long currentMaxAge = maxAge / 1000
							- System.currentTimeMillis() / 1000;
					if (currentMaxAge < 0) {
						currentMaxAge = 0;
					}
					httpResponse.addHeader(CacheFilter.HEADER_CACHE_CONTROL,
		...
		if (isContentGZiped()) {
			if (acceptsGZip) {
				((HttpServletResponse) response).addHeader(
						CacheFilter.HEADER_CONTENT_ENCODING, "gzip");
